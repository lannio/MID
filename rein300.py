...
def parse_args():
    ...
    parser.add_argument('--run_file',type=str, default='rein300.py')
    ...
    parser.add_argument('--type',type=str, default='mid-select') #updd mid mid-left only-left mid-select baseline
    ...
    parser.add_argument("--loss_gau_rate", default=0.00001, type=float)
    parser.add_argument("--loss_mean_rate", default=0.5, type=float)
    parser.add_argument("--loss_left_gau", default=0.001, type=float)
...
def get_select_fde_select(end_gau,end):
    endpoint=end_gau.reshape(256, 5)
    endpoint_oir=end.reshape(256, 2)

    sx = torch.exp(endpoint[:, 2])  # sx
    sy = torch.exp(endpoint[:, 3])  # sy
    corr = torch.tanh(endpoint[:, 4])  # corr
    cov = torch.zeros(endpoint.shape[0], 2, 2).to('cuda')
    cov[:, 0, 0] = sx * sx
    cov[:, 0, 1] = corr * sx * sy
    cov[:, 1, 0] = corr * sx * sy
    cov[:, 1, 1] = sy * sy
...
def get_select_fde(selected_end,end):
    ...
    fde=torch.mean((torch.norm((selected_end.reshape(256, 2) - end.reshape(256, 2)),dim=1)),dim=0)
...
def get_pred_de_mid(pred_list, gt):
    gt = gt.permute(1,0,2)*0.4
    V_y_rel_to_abs =  torch.cumsum((gt), dim=0) # [12, num_person, 2]
    kstep_V_pred_ls = []
    for i in range(20):
        kstep_V_pred_ls.append(torch.cumsum((pred_list[i].permute(1,0,2)*0.4), dim=0))  # cat [12, num_person, 2]
...
def find_end(gauss_param_tensor, coordinates_list):
    if(args.type=='mid-left'):
        return gauss_param_tensor 
    elif(args.type=='mid-select'):
        sampled_coords_tensor = torch.stack(coordinates_list)
        target =torch.unsqueeze((torch.squeeze(gauss_param_tensor)), 0)
        # 计算每个采样坐标与真实坐标之间的距离
        # print(sampled_coords_tensor.size())
        # print(target.size())
...
class Model_Dim_Up(Module):
    def __init__(self):
        ...
        self.cnn_up = MLP(input_dim = 2, output_dim = 5, hidden_size=[8,32])
...
class Model_backbone(Module):
    def __init__(self):
        ...
        context_dim=256
        if (args.type=='mid' or  args.type=='mid-left' or  args.type=='mid-select'):
            dim=2
        else:
            dim=5
...
class Model_his_to_end(Module):
    def __init__(self):
        ...
        # bs, 128 --> bs, 5
        if(args.type=='only-left'):
            dim=2
        else:
            dim=5

        self.encoder_his = MLP(input_dim = 128, output_dim = dim, hidden_size=[32, 8])
...
class Model_all_to_pred(Module):
    def __init__(self):
        ...
        context_dim=256
        self.end_encoder = MLP(input_dim = 2, output_dim = 128, hidden_size=[8,32])

        self.his_pred = MLP(input_dim = 1, output_dim = 12, hidden_size=[4,8])

        self.encoder_end = MLP(input_dim = 2, output_dim = 128, hidden_size=[8,32])

        self.pos_emb = PositionalEncoding(d_model=2*context_dim, dropout=0.1, max_len=24)
        self.concat1 = ConcatSquashLinear(256,2*context_dim,context_dim)
        self.layer = nn.TransformerEncoderLayer(d_model=2*context_dim, nhead=4, dim_feedforward=4*context_dim)
        self.transformer_encoder = nn.TransformerEncoder(self.layer, num_layers=3)
    ...
    def forward(self, his, end, his_feat, nei_feat):
        ...
        end_feat=self.end_encoder(end)
        feat_all= torch.concat((his_feat.view(bs, 1,-1),end_feat.view(bs,1, -1)),dim=2) #bs,1,256

        # his=his[:,:,2:4]
        his_pred=feat_all.permute(0,2,1)
        his_pred=self.his_pred(his_pred)
        his_all=his_pred.permute(0,2,1)
...
class Model_diffusion(Module):
    ...
    def get_loss(self, model_up, endpoint, his,nei,mask,edge_type,his_en,nei_en):
        ...
        if(args.type=='mid' or args.type=='mid-left' or args.type=='mid-select'):
            endpoint=endpoint 
            loss_gau = torch.tensor([0.0]).cuda()
    ...
    def sample(self, model_up, his,nei,mask,edge_type,his_en,nei_en):
        ...
        # if (args.type=='updd' or args.type=='baseline'):
        #     point_dim=5
        num_points=args.pred_length
        if(args.type=='updd' or args.type=='mid'):
            num_points=12
        self.alphas_cumprod = self.var_sched.alpha_bars

        his_feat=his_en(his)
        nei_feat=nei_en(his, nei, mask, edge_type, his_feat)
        context= torch.concat((his_feat,nei_feat),dim=1)

        if(args.type=='mid'):
            args.diffusion_sample_num=20


        for i in range(args.diffusion_sample_num):
            ...
            for t in range(self.var_sched.num_steps, 0, -1):
                ...
                e_theta = self.backbone(x_t, beta, context)
        ...
        return traj_list,his_feat,nei_feat,context

    def sample_updd(self, model_up, his,nei,mask,edge_type,his_en,nei_en):
        gau_up=model_up
        ...
        for diff_sample_num in range(args.diffusion_sample_num):
            ...
            if(args.type=='mid' or args.type=='mid-left' or args.type=='mid-select' or args.type=='updd' or args.type=='baseline'):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for node_type, data_loader in train_data_loader.items():
                ...
                for batch in pbar:
                    ...
                    if (args.type=='updd' or args.type=='mid'):
                        right_loss1,right_loss2,right_loss3, end_feat, his_feat,nei_feat,guide = self.model_diffuion.get_loss(self.gau_up, self.gt, self.his,self.nei, self.nei_mask,edge_type,self.his_en,self.nei_en)
                    else:
                    ...
                    train_loss_right = right_loss1*args.loss_diffusion_rate+right_loss2*args.loss_gau_rate
                    ...
                    if (args.type=='updd' or args.type=='mid'):
                        train_loss=train_loss_right
                    else:
                        end_list, end_gau=self.model_end(his_feat)
                        if (args.type=='only-left'):
                            selected_end=end_list[0]
                        else:    
                        ...
                        if (args.type=='only-left'):
                            ...
                            select_fde=get_select_fde_select(end_gau,self.end)
                            select_fde_sum=select_fde_sum+select_fde.item()
                        ...
                        train_loss_left=left_loss1*args.loss_left_gau
                        if (args.type=='only-left'):
                            train_loss = select_fde + train_loss_left
                        else: 
                            if(args.type=='mid-select'): 
                                train_loss = train_loss_right + train_loss_left + select_fde*args.loss_left_gau
                            else:
                                train_loss = train_loss_right + train_loss_left
            ...
            logger.info(f"Epoch {epoch}, {node_type} Right-Loss: {(right_loss/epoch):.2f}, Right-difusion: {(right_loss_diff/epoch):.2f}, Right-Gau: {(right_loss_gau/epoch):.2f}, Right-MSE: {(right_loss_mean/epoch):.2f}, train_time: {(ftime):.2f}, train_time_avg: {(ftimesum/epoch):.2f}")
            logger.info(f"Epoch {epoch}, {node_type} Left-Loss: {(left_loss/epoch):.2f}, Left-Gau: {(left_loss_gau/epoch):.2f}, train_time: {(ftime):.2f}, train_time_avg: {(ftimesum/epoch):.2f}")
            logger.info(f"Epoch {epoch}, {node_type},Select_FDE: {(select_fde_sum/epoch):.2f} ")
            ...
            with torch.no_grad():
                ...
                for node_type_test, data_loader_test in eval_data_loader.items():
                    ...
                    for test_batch in pbar2:
                        ...
                        if(args.type=='updd'):
                            ade, fde = get_pred_de(traj_pred, self.test_gt)
                        elif(args.type=='mid'):
                            ade, fde = get_pred_de_mid(traj_pred_list, self.test_gt)
                        else:
                            end_list, end_gau=self.model_end(his_feat)
                            if (args.type=='only-left'):
                                selected_end=end_list[0]
            ...
            if (ade_final>ade_avg):
                ade_final = ade_avg
                ade_epoch = epoch
                save_path = output_dir +'/model_ade.pt'
                torch.save({
            ...
            if (fde_final>fde_avg):
                fde_final=fde_avg
                fde_epoch = epoch
                save_path = output_dir +'/model_fde.pt'
                torch.save({
                            'gau_up': self.gau_up.state_dict(),
                            'his_en': self.his_en.state_dict(),
                            'nei_en': self.nei_en.state_dict(),
