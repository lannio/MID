...
def parse_args():
    ...
    parser.add_argument('--run_file',type=str, default='rein50.py')
    ...
    parser.add_argument('--exp',type=str, default='50-noGuide')
...
class ConcatSquashLinear2(Module):
    def __init__(self, dim_in, dim_out, dim_ctx):
        super(ConcatSquashLinear2, self).__init__()
        self._layer = Linear(dim_in, dim_out)
        self._hyper_bias = Linear(dim_ctx, dim_out, bias=False)
        self._hyper_gate = Linear(dim_ctx, dim_out)
...
class Model_all_to_pred(Module):
    def __init__(self):
        ...
        self.pos_emb = PositionalEncoding(d_model=2*context_dim, dropout=0.1, max_len=24)
        self.concat1 = ConcatSquashLinear2(dim,2*context_dim,context_dim)
        self.layer = nn.TransformerEncoderLayer(d_model=2*context_dim, nhead=4, dim_feedforward=4*context_dim)
        self.transformer_encoder = nn.TransformerEncoder(self.layer, num_layers=3)
        self.concat3 = ConcatSquashLinear2(2*context_dim,context_dim,context_dim)
        self.concat4 = ConcatSquashLinear2(context_dim,context_dim//2,context_dim)
        self.linear = ConcatSquashLinear2(context_dim//2, 5, context_dim)
