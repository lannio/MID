...
def parse_args():
    ...
    parser.add_argument('--run_file',type=str, default='rein10-3.py')
...
def find_end2(true_coords, sampled_coords_list):
    true_coords=true_coords.reshape(256,2)
    # 转换为合并的张量，以便于进行向量化操作
    sampled_coords_combined = torch.stack(sampled_coords_list).reshape(20,256,2)
    ...
    return selected_coords.squeeze() 
...
class Model_diffusion(Module):
    ...
    def get_loss(self, model_up, endpoint, his,nei,mask,edge_type,his_en,nei_en):
        ...
        return loss_diffusion ,0 ,0, endpoint, his_feat,nei_feat,guide
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for node_type, data_loader in train_data_loader.items():
                ...
                for batch in pbar:
                    ...
                    right_loss_gau = right_loss_gau + right_loss2
                    right_loss_mean = right_loss_mean + right_loss3
                    ...
                    selected_end=find_end2(end_feat, end_list)
                    # left_loss3 = 1/probs
                    ...
                    train_loss_left=left_loss1+train_loss_right
            ...
            with torch.no_grad():
                ...
                for node_type_test, data_loader_test in eval_data_loader.items():
                    ...
                    for test_batch in pbar2:
                        ...
                        selected_end=find_end2(traj_pred, end_list)
